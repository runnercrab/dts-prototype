...
import { getRoadmapData } from "@/lib/dts/getRoadmapData";
...
export default async function DtsRoadmapPage({ params }: { params: Promise<{ assessmentId: string }> }) {
  ...
  // TEMP firewall until 1A motor is fixed — production only
  // En producción real NO ejecutamos getRoadmapData (motor live zombie-vulnerable):
  // mostramos una pantalla intencional de "plan en construcción". En local/preview
  // (VERCEL_ENV !== 'production') el roadmap real se sigue viendo con normalidad.
    ...
    return (
      <div className="min-h-screen bg-[#f7f9fb] flex">
        ...
        <div className="ml-0 md:ml-[220px] flex-1 flex flex-col min-h-screen overflow-x-hidden">
          ...
          <main className="flex-1 px-4 md:px-6 py-10 md:py-12 pb-24 flex items-center justify-center">
            <div className="bg-white rounded-2xl p-10 md:p-14 max-w-xl text-center" style={{ border: "1.5px solid #dde3eb" }}>
              <div className="w-16 h-16 mx-auto mb-7 rounded-2xl flex items-center justify-center" style={{ backgroundColor: "#e8f4ff" }}>
                <svg className="w-8 h-8" style={{ color: GAPPLY_BLUE }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l4 2m6-2a10 10 0 11-20 0 10 10 0 0120 0z" />
                </svg>
              </div>
              <h1 className="text-[26px] md:text-[30px] font-extrabold text-slate-900 tracking-tight mb-4">
                Tu plan de acción se está finalizando
              </h1>
              <p className="text-[16px] md:text-[18px] text-slate-600 leading-relaxed">
                Estamos actualizando el motor de recomendaciones para mostrarte un plan más fiable. Te avisaremos cuando esté listo.
              </p>
              <div className="pt-9 mt-9" style={{ borderTop: "1.5px solid #dde3eb" }}>
  ...
  let data;
  try {
    data = await getRoadmapData(assessmentId);
  } catch {
    return (
      <div className="min-h-screen bg-[#f7f9fb] flex items-center justify-center">
        <div className="bg-white rounded-2xl p-10 max-w-md" style={{ border: '1.5px solid #dde3eb' }}>
          ...
          <p className="mt-3 text-[16px] text-slate-700">No se pudo cargar el plan de acción.</p>
