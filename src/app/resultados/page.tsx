...
function normalizeTab(v: any): TabKey | null {
  ...
  if (raw === 'overview' || raw === 'frenos' || raw === 'priorizacion') return raw
  return null
}
...
export default async function ResultadosPage({
  ...
  // ✅ REDIRECT: /resultados?assessmentId=... -> /resultados/<id>
  if (assessmentId && isUuid(assessmentId)) {
    const qs = tab ? `?tab=${tab}` : ''
    redirect(`/resultados/${assessmentId}${qs}`)
  }

  // ❌ Fallback si entran sin id o con id inválido
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-6">
      <div className="max-w-xl w-full bg-white border border-gray-200 rounded-xl p-6">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
          ...
          <div className="mt-2">
            (Te redirigirá automáticamente a <span className="font-mono">/resultados/&lt;assessmentId&gt;</span>)
          </div>
        </div>
        ...
        <div className="mt-5 flex items-center gap-3">
          ...
          <span className="text-xs text-slate-500">o abre resultados con un assessmentId válido.</span>
        </div>
      </div>
    </div>
  )
